...
int HoughCircle::GetCircles(
    int threshold, std::vector<std::pair<std::pair<int, int>, int> >& result) {
  int found = 0;
  if (_accu == 0) return found;
  for (int b = 0; b < _accu_h; b++) {
    for (int a = 0; a < _accu_w; a++) {
      if ((int)_accu[(b * _accu_w) + a] >= threshold) {
        ...
        if (max > (int)_accu[(b * _accu_w) + a]) continue;

        result.push_back(
            std::pair<std::pair<int, int>, int>(std::pair<int, int>(a, b), _r));
        found++;
      }
    }
  }
